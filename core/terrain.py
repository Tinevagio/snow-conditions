...
def get_terrain_grid(
    ...
    resolution_m : espacement entre les points (défaut 500m).

    Usage :
        grid = get_terrain_grid(45.90, 6.85, 45.95, 6.95)
        for p in grid:
            print(p.lat, p.lon, p.elevation_m, p.aspect_label(), p.slope_deg)
    """
    # Conversion resolution_m → degrés (~111km/°)
    ...
    while lat <= lat_max:
        lon = lon_min
        while lon <= lon_max:
            points.append(get_terrain_data(lat, lon, tiff_path=tiff_path))
            lon += delta_lon
        lat += delta_lat
